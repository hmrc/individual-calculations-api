...
class Def1_DefaultCarriedForwardLossSpec extends UnitSpec {
  ...
  def model(incomeSourceType: Def1_IncomeSourceType): Def1_DefaultCarriedForwardLoss =
    Def1_DefaultCarriedForwardLoss(
      incomeSourceId = "123456789012345",
      incomeSourceType = incomeSourceType,
    ...
    "successfully read in a model" when {
      ...
      testData.foreach { case Test(downstreamIncomeSourceType, incomeSourceType) =>
        s"provided downstream income source type $downstreamIncomeSourceType" in {
          downstreamJson(downstreamIncomeSourceType).as[Def1_DefaultCarriedForwardLoss] shouldBe
