...
class Def1_CarriedForwardLossSpec extends UnitSpec {
  ...
  def model(incomeSourceType: Def1_IncomeSourceType, claimType: Def1_ClaimType): Def1_CarriedForwardLoss =
    Def1_CarriedForwardLoss(
    ...
    "successfully read in a model" when {
      ...
      testData.foreach { case Test(downstreamIncomeSourceType, incomeSourceType, downstreamClaimType, claimType) =>
        s"provided downstream type of claim $downstreamClaimType" in {
          downstreamJson(downstreamIncomeSourceType, downstreamClaimType).as[Def1_CarriedForwardLoss] shouldBe
