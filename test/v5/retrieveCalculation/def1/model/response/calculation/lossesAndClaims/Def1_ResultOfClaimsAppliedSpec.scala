...
class Def1_ResultOfClaimsAppliedSpec extends UnitSpec {
  ...
  def model(incomeSourceType: Def1_IncomeSourceType, claimType: Def1_ClaimType): Def1_ResultOfClaimsApplied =
    Def1_ResultOfClaimsApplied(
  ...
  def mtdJson(incomeSourceType: Def1_IncomeSourceType, claimType: Def1_ClaimType): JsValue = Json.parse(s"""
       |{
       |  "claimId": "123456789012345",
       |  "originatingClaimId": "123456789012346",
       |  "incomeSourceId": "123456789012347",
  ...
  case class Test(downstreamIncomeSourceType: String, incomeSourceType: Def1_IncomeSourceType, downstreamClaimType: String, claimType: Def1_ClaimType)

  val testData: Seq[Test] = Seq[Test](
    Test("01", Def1_IncomeSourceType.`self-employment`, "CF", Def1_ClaimType.`carry-forward`),
    Test("02", Def1_IncomeSourceType.`uk-property-non-fhl`, "CSGI", Def1_ClaimType.`carry-sideways`),
    Test("03", Def1_IncomeSourceType.`foreign-property-fhl-eea`, "CFCSGI", Def1_ClaimType.`carry-forward-to-carry-sideways`),
    Test("04", Def1_IncomeSourceType.`uk-property-fhl`, "CSFHL", Def1_ClaimType.`carry-sideways-fhl`),
    Test("15", Def1_IncomeSourceType.`foreign-property`, "CB", Def1_ClaimType.`carry-backwards`),
    Test("01", Def1_IncomeSourceType.`self-employment`, "CBGI", Def1_ClaimType.`carry-backwards-general-income`)
  )
    ...
    "successfully read in a model" when {
      ...
      testData.foreach { case Test(downstreamIncomeSourceType, incomeSourceType, downstreamClaimType, claimType) =>
        s"provided downstream type of claim $downstreamClaimType" in {
          downstreamJson(downstreamIncomeSourceType, downstreamClaimType).as[Def1_ResultOfClaimsApplied] shouldBe
