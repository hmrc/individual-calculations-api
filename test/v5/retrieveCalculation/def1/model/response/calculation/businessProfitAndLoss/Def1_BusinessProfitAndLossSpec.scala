...
class Def1_BusinessProfitAndLossSpec extends UnitSpec {
  ...
  def model(incomeSourceType: Def1_IncomeSourceType): Def1_BusinessProfitAndLoss =
    ...
    "successfully read in a model" when {
      ...
      testData.foreach { case Test(downstreamIncomeSourceType, incomeSourceType) =>
        s"provided downstream income source type $downstreamIncomeSourceType" in {
          downstreamJson(downstreamIncomeSourceType).as[Def1_BusinessProfitAndLoss] shouldBe
