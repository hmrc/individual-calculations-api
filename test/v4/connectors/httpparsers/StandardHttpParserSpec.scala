...
class StandardHttpParserSpec extends UnitSpec {
  ...
  import v4.connectors.httpparsers.StandardHttpParser._
  ...
  // WLOG if Reads tested elsewhere
  case class SomeDownstreamModel(data: String) extends DownstreamResponse

  object SomeDownstreamModel {
    implicit val reads: Reads[SomeDownstreamModel] = Json.reads[SomeDownstreamModel]
  }
  ...
  val downstreamModel: SomeDownstreamModel                     = SomeDownstreamModel(data)
  val downstreamResponse: ResponseWrapper[SomeDownstreamModel] = ResponseWrapper(correlationId, downstreamModel)
  ...
  val multipleDownstreamErrorsJson: JsValue = Json.parse(
    """
      |{
      |   "failures": [
      |       {
  ...
  def getCorrelationIdMap(isDownstreamResponse: Boolean): Map[String, Seq[String]] =
    if (isDownstreamResponse) Map("CorrelationId" -> Seq(correlationId)) else Map("X-CorrelationId" -> Seq(correlationId))
  ...
  "The generic HTTP parser (from MTD)" when {
    "no status code is specified" must {
      ...
      "return a Right backend response containing the model object if the response json corresponds to a model object" in {
        val httpResponse = HttpResponse(OK, expectedJson.toString(), getCorrelationIdMap(isDownstreamResponse = false))
      ...
      "return an outbound 500 DownstreamError if a model object cannot be read from the response json" in {
        ...
        val httpResponse              = HttpResponse(OK, badFieldTypeJson.toString(), getCorrelationIdMap(isDownstreamResponse = false))
      ...
      handleErrorsCorrectly(httpReads, isDownstreamResponse = false)
      handleInternalErrorsCorrectly(httpReads, isDownstreamResponse = false)
      handleUnexpectedResponse(httpReads, isDownstreamResponse = false)
    }
  ...
  "The generic HTTP parser (from DOWNSTREAM)" when {
    "no status code is specified" must {
      ...
      "return an outbound error if a model object cannot be read from the response json" in {
        val badFieldTypeJson: JsValue = Json.obj("incomeSourceId" -> 1234, "incomeSourceName" -> 1234)
        val httpResponse              = HttpResponse(OK, badFieldTypeJson.toString(), getCorrelationIdMap(isDownstreamResponse = true))
        val expected                  = ResponseWrapper(correlationId, OutboundError(INTERNAL_SERVER_ERROR, DownstreamError))

        httpReads.read(method, url, httpResponse) shouldBe Left(expected)
      }

      handleErrorsCorrectly(httpReads, isDownstreamResponse = true)
      handleInternalErrorsCorrectly(httpReads, isDownstreamResponse = true)
      handleUnexpectedResponse(httpReads, isDownstreamResponse = true)
    ...
    "a success code is specified" must {
      "use that status code for success" in {
        implicit val successCode: SuccessCode                         = SuccessCode(PARTIAL_CONTENT)
        val httpReads: HttpReads[BackendOutcome[SomeDownstreamModel]] = implicitly
        val httpResponse = HttpResponse(PARTIAL_CONTENT, expectedJson.toString(), getCorrelationIdMap(isDownstreamResponse = true))
  ...
  "The generic HTTP parser for empty response (from MTD)" when {
    "no status code is specified" must {
      ...
      "receiving a 204 response" should {
        "return a Right backend response with the correct correlationId and no responseData" in {
          val httpResponse = HttpResponse(NO_CONTENT, JsObject.empty, getCorrelationIdMap(isDownstreamResponse = false))
    ...
    "a success code is specified" must {
      ...
      "use that status code for success" in {
        val httpResponse = HttpResponse(PARTIAL_CONTENT, JsObject.empty, getCorrelationIdMap(isDownstreamResponse = false))
  ...
  "The generic HTTP parser for empty response (from DOWNSTREAM)" when {
    "no status code is specified" must {
      ...
      "receiving a 204 response" should {
        "return a Right backend response with the correct correlationId and no responseData" in {
          val httpResponse = HttpResponse(NO_CONTENT, JsObject.empty, getCorrelationIdMap(isDownstreamResponse = true))

          httpReads.read(method, url, httpResponse) shouldBe Right(ResponseWrapper(correlationId, DownstreamUnit))
        }
      }
    ...
    "a success code is specified" must {
      ...
      "use that status code for success" in {
        val httpResponse = HttpResponse(PARTIAL_CONTENT, JsObject.empty, getCorrelationIdMap(isDownstreamResponse = true))
  ...
  private def handleErrorsCorrectly[A](httpReads: HttpReads[BackendOutcome[A]], isDownstreamResponse: Boolean): Unit =
    Seq(BAD_REQUEST, NOT_FOUND, FORBIDDEN, CONFLICT).foreach(statusCode =>
      s"receiving a $statusCode response" should {
        "be able to parse a single error with the same status code" in {
          val httpResponse = HttpResponse(statusCode, singleErrorJson.toString(), getCorrelationIdMap(isDownstreamResponse))
        ...
        "be able to parse multiple errors with the same status code" in {
          val getMultipleErrorsJson: String = if (isDownstreamResponse) multipleDownstreamErrorsJson.toString() else multipleErrorsJson.toString()
          val httpResponse                  = HttpResponse(statusCode, getMultipleErrorsJson, getCorrelationIdMap(isDownstreamResponse))

          val errors: List[BackendErrorCode] = List(BackendErrorCode("CODE"), BackendErrorCode("CODE 1"), BackendErrorCode("CODE 2"))
          val getMultipleErrorsResponse: List[BackendErrorCode] = if (isDownstreamResponse) errors.tail else errors
        ...
        "return an outbound 500 DownstreamError when errors are returned without a primary error" in {
          val httpResponse = HttpResponse(statusCode, missingPrimaryErrorJson.toString(), getCorrelationIdMap(isDownstreamResponse))
        ...
        "return an outbound 500 DownstreamError when the error returned doesn't match the Error model" in {
          val httpResponse = HttpResponse(statusCode, malformedErrorJson.toString(), getCorrelationIdMap(isDownstreamResponse))
  ...
  private def handleInternalErrorsCorrectly[A](httpReads: HttpReads[BackendOutcome[A]], isDownstreamResponse: Boolean): Unit =
    Seq(INTERNAL_SERVER_ERROR, SERVICE_UNAVAILABLE).foreach(responseCode =>
      s"receiving a $responseCode response" should {
        "return an outbound 500 DownstreamError when the error returned matches the Error model" in {
          val httpResponse = HttpResponse(responseCode, singleErrorJson.toString(), getCorrelationIdMap(isDownstreamResponse))
        ...
        "return an outbound 500 DownstreamError when the error returned doesn't match the Error model" in {
          val httpResponse = HttpResponse(responseCode, malformedErrorJson.toString(), getCorrelationIdMap(isDownstreamResponse))
  ...
  private def handleUnexpectedResponse[A](httpReads: HttpReads[BackendOutcome[A]], isDownstreamResponse: Boolean): Unit =
