...
package v9.retrieveCalculation.def4

import api.models.domain.{CalculationId, Nino, TaxYear}
import api.models.errors.*
import api.utils.UnitSpec
import v9.retrieveCalculation.models.request.Def4_RetrieveCalculationRequestData
...
class Def4_RetrieveCalculationValidatorSpec extends UnitSpec {
  ...
  private def validator(nino: String, taxYear: String, calculationId: String) =
    new Def4_RetrieveCalculationValidator(nino, taxYear, calculationId)

  "validator" should {
    "return the parsed domain object" when {
      "a valid request is supplied" in {
        val result = validator(validNino, validTaxYear, validCalculationId).validateAndWrapResult()
        result.shouldBe(Right(Def4_RetrieveCalculationRequestData(parsedNino, parsedTaxYear, parsedCalculationId)))
      }
    }
