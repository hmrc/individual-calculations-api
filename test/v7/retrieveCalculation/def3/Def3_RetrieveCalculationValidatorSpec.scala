...
import v7.retrieveCalculation.models.request.Def3_RetrieveCalculationRequestData

class Def3_RetrieveCalculationValidatorSpec extends UnitSpec {
  ...
  private def validator(nino: String, taxYear: String, calculationId: String) =
    new Def3_RetrieveCalculationValidator(nino, taxYear, calculationId)

  "validator" should {
    "return the parsed domain object" when {
      "a valid request is supplied" in {
        val result = validator(validNino, validTaxYear, validCalculationId).validateAndWrapResult()
        result shouldBe Right(Def3_RetrieveCalculationRequestData(parsedNino, parsedTaxYear, parsedCalculationId))
      }
    }
    ...
    "return TaxYearFormatError error" when {
      "an invalid tax year is supplied" in {
        val result = validator(validNino, "202526", validCalculationId).validateAndWrapResult()
        result shouldBe Left(
