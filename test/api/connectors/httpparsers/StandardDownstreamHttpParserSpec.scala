    ...
    Seq(BAD_REQUEST, NOT_FOUND, FORBIDDEN, CONFLICT).foreach(responseCode =>
      s"receiving a $responseCode response" should {
        ...
        "return an outbound error when the error returned doesn't match the Error model" in {
          ...
          httpReads.read(method, url, httpResponse) shouldBe Left(ResponseWrapper(correlationId, errors.OutboundError(errors.InternalError)))
        }
      })
      ...
      "return an outbound BUSINESS_ERROR error containing the BVR ids" in {
        ...
        httpReads.read(method, url, httpResponse) shouldBe
          Left(
            ResponseWrapper(
              correlationId,
              errors.OutboundError(BVRError, Some(Seq(MtdError("BVR1", "", BAD_REQUEST), MtdError("BVR2", "", BAD_REQUEST))))))
      }
    }
  }
