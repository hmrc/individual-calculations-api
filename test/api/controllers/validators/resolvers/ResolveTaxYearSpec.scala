...
class ResolveTaxYearSpec extends UnitSpec with ResolverSupport {
  ...
  "ResolveTysTaxYear" should {
    "return no errors" when {
      ...
      "given a valid tax year that's above or equal to TaxYear.tysTaxYear in an Option" in {
        val validTaxYear = "2023-24"
        ResolveTysTaxYear(Some(validTaxYear)) shouldBe Valid(Some(TaxYear.fromMtd(validTaxYear)))
      }

      "given no tax year" in {
        ResolveTysTaxYear(None) shouldBe Valid(None)
      }
    }
