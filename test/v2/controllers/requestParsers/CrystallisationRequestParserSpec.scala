  ...
  "parse" when {
    ...
    "single validation error" should {
      "return the error" in new Test {
        val data: CrystallisationRawData =
          CrystallisationRawData("invalidNino", taxYear, AnyContentAsJson(Json.obj("calculationId" -> calculationId)))
    ...
    "multiple validation errors" should {
      "return the errors" in new Test {
        val data: CrystallisationRawData =
          CrystallisationRawData("invalidNino", "2018-20", AnyContentAsJson(Json.obj("calculationId" -> calculationId)))
