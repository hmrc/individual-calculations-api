...
class RetrieveCalculationMetadataParserSpec extends UnitSpec {
  ...
  trait Test extends MockRetrieveCalculationMetadataValidator {
    lazy val parser = new RetrieveCalculationMetadataParser(mockValidator)
  }

  "parse" when {
    "valid input" should {
      "parse the request" in new Test {
        val data = RetrieveCalculationMetadataRawData(nino, calculationId)
        MockValidator.validate(data).returns(Nil)

        parser.parseRequest(data) shouldBe Right(RetrieveCalculationMetadataRequest(Nino(nino), calculationId))
      }
    }
  }
  ...
  "multiple validation errors" should{
    "return the errors" in new Test{
      val data = RetrieveCalculationMetadataRawData("AA111111F","f2fb30e5-4ab6-4a29-b3c1-c7264")
