...
class CalculationListItemSpec extends UnitSpec {

  "CalculationListItem" when {
    "read from valid JSON" should {
      "produce the expected CalculationListItem" in {
        listItemJson.as[CalculationListItem] shouldBe listItemResponse
      }
    }

    "written to JSON" must {
      "produce the expected JsObject" in {
        Json.toJson(listItemResponse) shouldBe listItemJson
      }
    }
  }
}
