...
class Def1_SubmitFinalDeclarationValidator(nino: String, taxYear: String, calculationId: String)
  ...
  private val resolveTaxYear = ResolveTaxYear.resolver.thenValidate(satisfiesMax(maximumSupportedTaxYear, RuleTaxYearForVersionNotSupportedError))
  ...
  def validate: Validated[Seq[MtdError], SubmitFinalDeclarationRequestData] =
    ...
    ).mapN(Def1_SubmitFinalDeclarationRequestData.apply)
