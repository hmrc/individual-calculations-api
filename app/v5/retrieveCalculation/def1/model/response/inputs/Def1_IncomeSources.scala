...
case class Def1_IncomeSources(businessIncomeSources: Option[Seq[Def1_BusinessIncomeSource]],
                              nonBusinessIncomeSources: Option[Seq[Def1_NonBusinessIncomeSource]]) {

  val isDefined: Boolean =
    !(businessIncomeSources.isEmpty && nonBusinessIncomeSources.isEmpty)

  def withoutCessationDate: Def1_IncomeSources =
  ...
  def withoutCommencementDate: Def1_IncomeSources = businessIncomeSources match {
    case Some(det) =>
...
object Def1_IncomeSources {
  implicit val format: OFormat[Def1_IncomeSources] = Json.format[Def1_IncomeSources]
}
