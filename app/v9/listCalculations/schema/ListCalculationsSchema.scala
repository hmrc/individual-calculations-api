...
package v9.listCalculations.schema

import api.controllers.validators.resolvers.ResolveTaxYear
import api.models.domain.TaxYear
import api.schema.DownstreamReadable
import play.api.libs.json.Reads
import v9.listCalculations.def1.model.response.Def1_Calculation
import v9.listCalculations.def2.model.response.Def2_Calculation
import v9.listCalculations.def3.model.response.Def3_Calculation
import v9.listCalculations.model.response.*
...
object ListCalculationsSchema {
  ...
  def schemaFor(taxYear: String): ListCalculationsSchema = {
    ...
    val schema: Option[ListCalculationsSchema] = resolveTaxYear.map(schemaFor)
