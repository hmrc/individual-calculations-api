...
import v9.common.model.domain.{CalculationType, `final-declaration`, `in-year`, `intent-to-finalise`}
...
import v9.listCalculations.model.request.{Def2_ListCalculationsRequestData, ListCalculationsRequestData}
...
class Def2_ListCalculationsValidator(nino: String, taxYear: String, calculationType: Option[String]) extends Validator[ListCalculationsRequestData] {
  ...
  def validate: Validated[Seq[MtdError], ListCalculationsRequestData] =
    ...
    ).mapN(Def2_ListCalculationsRequestData.apply)
